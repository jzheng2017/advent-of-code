...
public class Day5 {
    ...
    private static void part1() {
        List<Stack> crates = createStackOfCrates();
        ...
        for (String move : moves) {
            ...
            Stack<Character> fromStack = crates.get(from-1);
            Stack<Character> toStack = crates.get(to-1);
        ...
        for (Stack el : crates) {
            System.out.print(el.peek());
        }
    }

    private static List<Stack> createStackOfCrates() {
        ...
        IntStream.range(0, cratePositions.size()).forEach((x) -> stacks.add(new Stack()));

        for (int i = crates.size() - 2; i >= 0; i--) {
            String line = crates.get(i);
            for (int j = 0; j < cratePositions.size(); j++) {
                Stack stack = stacks.get(j);
                Character c = line.charAt(cratePositions.get(j));
