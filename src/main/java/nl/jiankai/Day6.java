...
public class Day6 {
    private static List<String> arr = Arrays.asList(("gsvdkufnoawjmhp\n" +
            "wvhusojpnikgfadb\n" +
            "vshnpfedgwajkou\n" +
            "aujodhskfvnpgw\n" +
            "okpdnwhsfvjguqa\n" +
    ...
    private static void part2() {
        ...
        for (String s : arr) {
            groups.add(Arrays.asList(s.split("\n")));
        }

        int[] answeredYes = new int[groups.size()];

        for (int i = 0; i < groups.size(); i++) {
            List<String> group = groups.get(i);
            String[] distinctChars = Arrays.stream((String.join("", group)).split("")).distinct().toArray(String[]::new);

            for (String distinctChar : distinctChars) {
                answeredYes[i]++;
                for (String person : group) {
                    if (!person.contains(distinctChar)) {
                        answeredYes[i]--;
                        break;
                    }
                }
        ...
        System.out.println(Arrays.stream(answeredYes).sum());
    }
}
