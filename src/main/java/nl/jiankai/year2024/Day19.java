...
public class Day19 {
    ...
    private static boolean possible(String[] patterns, String design, int offset) {
        if (offset == design.length()) {
            return true;
        }
        for (String pattern : patterns) {
            if (design.startsWith(pattern, offset)) {
                if (possible(patterns, design, offset + pattern.length())) {
                    return true;
                }
            }
        }
    ...
    private static void part2() {
        ...
        long ans = 0;
        for (String design : designs) {
            ans += perms(patterns, design, new long[design.length()], 0);
        }

        System.out.println(ans);
    }

    private static long perms(String[] patterns, String design, long[] dp, int offset) {
        if (offset == design.length()) {
            return 1;
        }
        ...
        for (String pattern : patterns) {
            if (design.startsWith(pattern, offset)) {
                tmp += perms(patterns, design, dp, offset + pattern.length());
            }
        }
