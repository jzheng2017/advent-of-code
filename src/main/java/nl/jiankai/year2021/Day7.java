...
public class Day7 {
    ...
    static void part1() {
        List<Integer> positions = input.stream().map(Integer::parseInt).collect(Collectors.toList());
        Collections.sort(positions);
        int fuel = Integer.MAX_VALUE;

        if (positions.size() % 2 == 0) {
            int m1 = positions.size() / 2 - 1;
            int m2 = m1 + 1;
            int p1 = positions.get(m1);
            int p2 = positions.get(m2);
            int localFuel = 0;
            for (Integer integer : positions) {
                localFuel += Math.abs(integer - p1);
            }
            ...
            for (Integer position : positions) {
                localFuel += Math.abs(position - p2);
            }
            if (localFuel < fuel) {
                fuel = localFuel;
            }
        } else {
            int middle = positions.size() / 2 - 1;
            int position = positions.get(middle);
            for (int i = 0; i < positions.size(); i++) {
                fuel += Math.abs(positions.get(i) - position);
            }
        }
    ...
    static void part2() {
        ...
        int highest = positions.stream().mapToInt(v->v).max().orElseThrow();

        for (int i = 0; i <= highest; i++) {
            long localFuel = 0;
            for (int p2 : positions) {
                long delta = Math.abs(i - p2);
                localFuel += (delta * (delta + 1)) / 2;
            }
            if (localFuel < fuel) {
                fuel = localFuel;
            }
...
//            long localFuel = 0;
//            for (Integer position : positions) {
//                long delta = Math.abs(position - p1);
//                localFuel += (delta * (delta + 1)) / 2;
//            }
//
//            if (localFuel < fuel) {
...
//            localFuel = 0;
//            for (Integer position : positions) {
//                long delta = Math.abs(position - p2);
//                localFuel += (delta * (delta + 1)) / 2;
//            }
//            if (localFuel < fuel) {
//                fuel = localFuel;
...
//            int middle = positions.size() / 2 - 1;
//            int position = positions.get(middle);
//            for (Integer secondPos : positions) {
//                long delta = Math.abs(position - secondPos);
